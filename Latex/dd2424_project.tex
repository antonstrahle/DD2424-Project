...
For our experiments we set up three different convolutional neural networks with varying degrees of accuracies on the full dataset, hereafter called base accuracies. The first networks has a basic CNN architecture which is presented in Figure X below (SKA VI KÖRA bref?). Beyond our own basic CNN we also used transfer learning to implement MobileNet and ResNet 50, both trained on ImageNet and specified towards our specific task of bird classification.
...
For the Basic CNN the structure of the layers are shown in Figure X

***motivation till de valda lagertyperna***
e.g We have chosen CNNs since they have shown to perform good, sometimes extremely good, on computer vision tasks. 
MaxPooling to ...
BatchNormalization has been a necessity to avoid exploding gradients and to keep the network stable. We have further used dropout to avoid overfitting which we have anticipated to encounter, both considering the fact that we have X million parameters and also that we will try the network on subsets with very few training examples. 
Further explanations about the exact order and parameters are provided in the experiments section.
